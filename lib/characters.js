import fallbackCharactersRaw from "../data/fallback-characters.json";
...
function shapeFallbackCharacter(record) {
  const gallery = asArray(record.gallery).map((url) => normalizeDriveUrl(url));
  const cover = normalizeDriveUrl(record.cover) || gallery[0];
  return {
    id: record.id || toSlug(record.name || "legend"),
    name: record.name || "Untitled Legend",
    alias: asArray(record.alias),
    gender: record.gender || null,
    alignment: record.alignment || null,
    locations: asArray(record.locations),
    status: record.status || null,
    era: record.era || null,
    firstAppearance: record.firstAppearance || null,
    powers: asArray(record.powers).map((power, index) => ({
      name: power?.name || `Power ${index + 1}`,
      level: Number.isFinite(power?.level) ? power.level : 0,
    })),
    faction: asArray(record.faction),
    tags: asArray(record.tags),
    shortDesc: record.shortDesc || "",
    longDesc: record.longDesc || "",
    stories: asArray(record.stories),
    cover,
    gallery,
  };
}
...
function getFallbackCharacters() {
  if (!fallbackProcessed) {
    fallbackProcessed = fallbackCharactersRaw.map((record) => fillDailyPowers(shapeFallbackCharacter(record)));
    if (!cache.data || !cache.data.length) {
      cache = { data: fallbackProcessed, timestamp: Date.now() };
    }
  }
...
export async function fetchCharactersFromSheets({ force = false } = {}) {
  ...
  if (!sheetId) {
    if (process.env.NODE_ENV !== "production") {
      console.warn("[characters] Google Sheets configuration missing; using bundled fallback data.");
    }
    return getFallbackCharacters();
  }
  const sheetNames = [process.env.LOREMAKER_SHEET_TAB || "Characters", ...FALLBACK_SHEET_NAMES];
  ...
  if (process.env.NODE_ENV !== "production") {
    console.warn("[characters] Falling back to bundled data after Sheets fetch failure", lastError);
  }
  const fallback = getFallbackCharacters();
  if (fallback && fallback.length) {
    return fallback;
  }
  throw lastError || new Error("Unable to load characters");
}
