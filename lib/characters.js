...
export function fillDailyPowers(c) {
  ...
  rawTags.forEach((tag) => {
    ...
    if (match) {
      ...
      eraTags.push(value && value.length ? value : "Era");
    } else {
      filteredTags.push(tag);
    }
  });
  ...
  const baseEra = (c.era || "").trim();
  if (baseEra && !uniqueEraTags.some((value) => value.toLowerCase() === baseEra.toLowerCase())) {
    uniqueEraTags.unshift(baseEra);
  }
  const resolvedEra = uniqueEraTags[0] || null;
